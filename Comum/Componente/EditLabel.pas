...
type
  ...
  TTipo = (Numero,Num_Decimal,Data,Texto,Pesquisa,CPF,CNPJ,Fone,FoneSemDDD,CEP,Hora,Data_Hora,CNPJ_CPF);
  ...
  public
    ...
    property FormattedText: string read GetText;
  end;
...
function TCustomEditLabel.GetText: string;
var str: string;
begin
  case Self._Tipo of
  Data, CNPJ, CPF:
    ...
    str := StringReplace(str,'-','',[rfReplaceAll]);
    if Self._Tipo = CNPJ then
      Result := LPAD(str,14,'0')
    else if Self._Tipo = CPF then
      Result := LPAD(str,11,'0')
    else
...
begin
  ...
  case FTipo of
    ...
    FoneSemDDD: EditMask := mskFONEsemDDD;
...
begin
  ...
  begin
    Case FTipo of
      ...
      CNPJ: Text := LPAD(Text,14,'0');
      CPF : Text := LPAD(Text,11,'0');
    end;
  end;
  Repaint;
  inherited;
...
procedure TCustomEditLabel.SetText(str: string);
begin
  FText := str;
  inherited Text := str;
end;
