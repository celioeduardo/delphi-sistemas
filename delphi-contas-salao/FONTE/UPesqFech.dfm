object frmPesqFech: TfrmPesqFech
  Left = 52
  Top = 50
  BorderIcons = []
  BorderStyle = bsDialog
  Caption = 'Pesquisa de Fechamento'
  ClientHeight = 435
  ClientWidth = 741
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poDefault
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object tab: TPageControl
    Left = 0
    Top = 0
    Width = 741
    Height = 379
    ActivePage = tabCadastro
    Align = alClient
    TabOrder = 0
    TabStop = False
    object tabCadastro: TTabSheet
      Caption = 'Fechamento'
      object pnl1: TPanel
        Left = 0
        Top = 0
        Width = 733
        Height = 351
        Align = alClient
        BevelInner = bvSpace
        BevelOuter = bvNone
        TabOrder = 0
        DesignSize = (
          733
          351)
        object grd: TDBGrid
          Left = 3
          Top = 5
          Width = 726
          Height = 342
          Anchors = [akLeft, akTop, akRight, akBottom]
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = ANSI_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Arial'
          TitleFont.Style = []
          OnDblClick = grdDblClick
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'NUM_MES'
              Title.Alignment = taCenter
              Title.Caption = 'M'#234's'
              Width = 30
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'NUM_ANO'
              Title.Alignment = taCenter
              Title.Caption = 'Ano'
              Width = 50
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'DAT_FECHAMENTO'
              Title.Alignment = taCenter
              Title.Caption = 'Data'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_OMTJ_RES'
              Title.Alignment = taRightJustify
              Title.Caption = 'Res. OMTJ'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_OMTJ'
              Title.Alignment = taRightJustify
              Title.Caption = 'OMTJ'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_OMTJ_TOTAL'
              Title.Alignment = taRightJustify
              Title.Caption = 'OMTJ Total'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_AJU_FIN'
              Title.Alignment = taRightJustify
              Title.Caption = 'Ajuda Fin.'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_FUNDO_SR'
              Title.Alignment = taRightJustify
              Title.Caption = 'Fundo S.R.'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_AUX_CAL'
              Title.Alignment = taRightJustify
              Title.Caption = 'Aux. Calmidade'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_CONTA_CONG'
              Title.Alignment = taRightJustify
              Title.Caption = 'Conta Cong.'
              Width = 80
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_SALAO_ASSEMB'
              Title.Alignment = taRightJustify
              Title.Caption = 'Sal. Assemb.'
              Width = 80
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOM_SALAO_ASSEMB'
              Title.Caption = 'Nome Sal. Ass.'
              Width = 90
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_OUTROS_DONAT'
              Title.Alignment = taRightJustify
              Title.Caption = 'Outros Donat.'
              Width = 80
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOM_OUTROS_DONAT'
              Title.Caption = 'Nome Out. Don.'
              Width = 90
              Visible = True
            end
            item
              Alignment = taRightJustify
              Expanded = False
              FieldName = 'VAL_DEP_CONG'
              Title.Alignment = taRightJustify
              Title.Caption = 'Dep'#243'sito Cong.'
              Width = 80
              Visible = True
            end>
        end
      end
      object txtHabilitaEdicao: TEdit
        Left = -568
        Top = 154
        Width = 121
        Height = 21
        TabOrder = 1
        Text = 'txtHabilitaEdicao'
      end
    end
    object tabLancFechamento: TTabSheet
      Caption = 'Lan'#231'amentos do Fechamento'
      ImageIndex = 1
      OnShow = tabLancFechamentoShow
      DesignSize = (
        733
        351)
      object lblMesAnoFechamento: TLabel
        Left = 5
        Top = 3
        Width = 724
        Height = 24
        AutoSize = False
        Caption = 'M'#202'S / ANO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object grdLanc: TDBGrid
        Left = 3
        Top = 29
        Width = 726
        Height = 313
        Anchors = [akLeft, akTop, akRight, akBottom]
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = ANSI_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Arial'
        TitleFont.Style = []
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'DAT_LANCAMENTO'
            Title.Alignment = taCenter
            Title.Caption = 'Data'
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'COD_TP_LANC'
            Title.Alignment = taCenter
            Title.Caption = 'Tipo'
            Width = 30
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DSC_LANCAMENTO'
            Title.Caption = 'Descri'#231#227'o'
            Width = 250
            Visible = True
          end
          item
            Alignment = taRightJustify
            Expanded = False
            FieldName = 'VAL_LANCAMENTO'
            Title.Alignment = taRightJustify
            Title.Caption = 'Valor'
            Width = 80
            Visible = True
          end
          item
            Alignment = taRightJustify
            Expanded = False
            FieldName = 'COD_CONTA_DEB'
            Title.Alignment = taRightJustify
            Title.Caption = 'Conta'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_CONTA_DEB'
            Title.Caption = 'Conta D'#233'bito'
            Width = 100
            Visible = True
          end
          item
            Alignment = taRightJustify
            Expanded = False
            FieldName = 'COD_CONTA_CRE'
            Title.Alignment = taRightJustify
            Title.Caption = 'Conta'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOM_CONTA_CRE'
            Title.Caption = 'Conta Cr'#233'dito'
            Width = 100
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'COD_CT'
            Title.Alignment = taCenter
            Title.Caption = 'CT'
            Width = 30
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUM_LANCAMENTO'
            Visible = False
          end
          item
            Alignment = taRightJustify
            Expanded = False
            Title.Alignment = taRightJustify
            Title.Caption = 'Num. Lan'#231'amento'
            Width = 100
            Visible = True
          end>
      end
    end
  end
  object pnl3: TPanel
    Left = 0
    Top = 379
    Width = 741
    Height = 56
    Align = alBottom
    BevelInner = bvRaised
    TabOrder = 1
    object btnLocaliza: TBitBtn
      Left = 30
      Top = 13
      Width = 97
      Height = 33
      Caption = '  &Localizar'
      TabOrder = 0
      OnClick = btnLocalizaClick
      Glyph.Data = {
        
        0000000000000000000000000000}
    end
    object btnSair: TBitBtn
      Left = 606
      Top = 13
      Width = 97
      Height = 33
      Caption = '  &Sair'
      TabOrder = 1
      OnClick = btnSairClick
      Glyph.Data = {
        
        0700000000000000000000000000}
    end
  end
end
